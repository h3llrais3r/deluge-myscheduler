...
class MySchedulerSelectWidget(Gtk.DrawingArea):
    def __init__(self, hover):
        super(MySchedulerSelectWidget, self).__init__()
        self.set_events(
...
class GtkUI(Gtk3PluginBase):
    ...
    def on_apply_prefs(self):
        ...
        config['ignore_schedule'] = self.check_ignore_schedule.get_active()
        config['force_use_individual'] = self.check_individual_scheduling.get_active()
        config['force_unforce_finished'] = self.check_unforce_finished.get_active()
    ...
    def on_show_prefs(self):
        def on_get_config(config):
            ...
            self.check_ignore_schedule.set_active(config['ignore_schedule'])
            self.check_individual_scheduling.set_active(config['force_use_individual'])
            self.check_unforce_finished.set_active(config['force_unforce_finished'])
    ...
    def create_prefs_page(self):
        # Select Widget
        hover = Gtk.Label()
        self.scheduler_select = MySchedulerSelectWidget(hover)
        ...
        frame = Gtk.Frame()
        label = Gtk.Label()
        label.set_markup(_('<b>Schedule Settings</b>'))
        frame.set_label_widget(label)
        frame.set_shadow_type(Gtk.ShadowType.NONE)
        # frame.set_margin_left(15)
        # frame.add(hbox)

        vbox_schedule_settings = Gtk.Box.new(Gtk.Orientation.VERTICAL, spacing=5)
        vbox_schedule_settings.set_margin_left(15)
        self.check_ignore_schedule = Gtk.CheckButton(_('Ignore Schedule'))
        vbox_schedule_settings.pack_start(self.check_ignore_schedule, False, False, 0)
        vbox_schedule_settings.pack_start(hbox, False, False, 0)
        vbox_schedule_settings.pack_start(hover, False, False, 0)
        ...
        frame.set_shadow_type(Gtk.ShadowType.NONE)
        # frame.set_margin_left(15)
        vbox_forced_settings = Gtk.Box.new(Gtk.Orientation.VERTICAL, spacing=5)
        vbox_forced_settings.set_margin_left(15)
        self.check_individual_scheduling = Gtk.CheckButton(_('Use Individual Scheduling'))
        vbox_forced_settings.pack_start(self.check_individual_scheduling, False, False, 0)
        self.check_unforce_finished = Gtk.CheckButton(_('Un-Force on Finished'))
        vbox_forced_settings.pack_start(self.check_unforce_finished, False, False, 0)
